...
exports.sendEmail = async (req, res) => {
    ...
    try {
        ...
        const found_email_query1 = 'SELECT * FROM users WHERE email = $1'
        const foundResult1 = await pool.query(found_email_query1, [email])

        if (foundResult.rows[0] || foundResult1.rows[0] )  {
...
const sendOTPVerificationEmail = async (email, res) => {
    try {
        ...
        console.log(foundStoredOtp.rows)
        ...
        let sendEmailResponse = await transporter.sendMail({
            ...
            html: emailOTPBody(otp, "Sport Development Media", "#FF5722")
